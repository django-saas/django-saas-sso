...
class TestOAuthLogin(FixturesTestCase):
    ...
    def generate_apple_id_token(self):
        key = RSAKey.import_key(self.load_fixture('rsa_key.pem'))
        now = int(time.time())
        claims = {
    ...
    def run_github_flow(self):
        params = self.resolve_url_params('/m/login/github/')
    ...
    def test_google_not_create_user(self):
        params = self.resolve_url_params('/m/login/google/')

        with self.mock_requests(
            'google_openid_configuration.json',
            'google_token.json',
            'google_user.json',
        ):
            state = params['state']
    ...
    def test_google_flow_with_preferred_username(self):
        ...
        with self.mock_requests('google_openid_configuration.json', 'google_token.json', 'google_user_pref.json'):
            state = params['state']
            resp = self.client.get(f'/m/auth/google/?state={state}&code=123')
            self.assertEqual(resp.status_code, 302)
    ...
    def test_google_flow_email_not_verified(self):
        ...
        with self.mock_requests('google_openid_configuration.json', 'google_token.json', 'google_user_unverified.json'):
            state = params['state']
            resp = self.client.get(f'/m/auth/google/?state={state}&code=123')
            self.assertEqual(resp.status_code, 302)
    ...
    def test_apple_flow(self):
        params = self.resolve_url_params('/m/login/apple/')

        # Test Apple's POST callback (form_post)
        with self.mock_requests('apple_openid_configuration.json', 'apple_jwks.json') as m:
